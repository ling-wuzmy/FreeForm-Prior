...
import sys
sys.path.append("D:/code/VirtualMarker")
from vm_ik.core.config import cfg, update_config, init_experiment_dir
from vm_ik.dataset.demo_dataset import DemoDataset
from vm_ik.utils.vis import render_mesh
from vm_ik.utils.funcs_utils import load_checkpoint
from vm_ik.utils.smpl_utils import get_smpl_faces
from vm_ik.utils.coord_utils import pixel2cam
...
def parse_args():
    ...
    parser.add_argument('--input_path',
                        default='/data2024/yyyl/zmy/VMarker-Pro/VirtualMarker/inputs/input/image_00311_27176.jpg',
                        help='path to the input data')
    parser.add_argument('--batch_size', type=int, default=1, help='batch size for detection and motion capture')
...
class Simple3DMeshInferencer:
    def __init__(self, args, load_path='', writer=None, img_path_list=[], detection_all=[], max_person=-1, fps=-1):
        ...
        self.fps = fps
        self.demo_dataloader = DataLoader(self.demo_dataset, batch_size=min(args.batch_size, len(detection_all)),
                                          num_workers=0)
    ...
    def infer(self, epoch, current_model=None):
        ...
        with torch.no_grad():
            for i, meta in enumerate(tqdm(self.demo_dataloader, dynamic_ncols=True)):
                ...
                _, _, _, _, pred_mesh, _, pred_root_xy_img,_, _ = self.model(imgs, inv_trans, intrinsic_param,
                                                                              pose_root, depth_factor, flip_item=None,
                                                                              flip_mask=None)
    ...
    def visualize(self, results):
        ...
        for img_idx, img_path in enumerate(tqdm(self.img_path_list, dynamic_ncols=True)):
            ...
            rgb, depth = render_mesh(ori_img_height, ori_img_width, pred_mesh_T / 1000.0, self.smpl_faces,
                                     {'focal': focal_T, 'princpt': center_pt_T})
            valid_mask = (depth > 0)[:, :, None]
            rendered_img = rgb * valid_mask + img[:, :, ::-1] * (1 - valid_mask)
            cv2.imwrite(osp.join(cfg.vis_dir, f"{self.args.input_path.split('/')[-1][:-4]}_results_in_2d.jpg"),
                        rendered_img.astype(np.uint8)[..., ::-1])
...
def detect_all_persons(args, img_dir):
    ...
    valid_frame_idx_all = []
    with torch.no_grad():
        for _, (inputs, targets_2d, weights_2d, targets_3d, meta, input_AGR) in enumerate(
                tqdm(infer_loader, dynamic_ncols=True)):
...
def main(args):
    ...
    inferencer = Simple3DMeshInferencer(args, load_path=load_path_test, writer=writer, img_path_list=img_path_list,
                                        detection_all=detection_all, max_person=max_person, fps=fps)
