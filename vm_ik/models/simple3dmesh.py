...
from vm_ik.models import simple3dpose
...
class Simple3DMesh(nn.Module):
    ...
    def forward(self, x, trans_inv, intrinsic_param, joint_root, depth_factor, flip_item=None, flip_output=False,
        ...
        pred_xyz_jts, confidence, pred_uvd_jts_flat, pred_root_xy_img, pred_uvd_jts_29_flat, joints_from_verts = self.simple3dpose(
            x, trans_inv, intrinsic_param, joint_root, depth_factor, flip_item, flip_output,
            flip_mask)  # (B, J+K, 3), (B, J+K)
        ...
        return pred_xyz_jts_ret, pred_uvd_jts_flat, adaptive_A, confidence_ret, mesh3d, None, pred_root_xy_img, pred_uvd_jts_29_flat, joints_from_verts


def get_model(mesh_num_joints, flip_pairs, vm_A=None, selected_indices=None):
    model = Simple3DMesh(mesh_num_joints=mesh_num_joints, flip_pairs=flip_pairs, vm_A=vm_A,
